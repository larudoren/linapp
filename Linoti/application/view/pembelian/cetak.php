<?php
	...
	$pdf->Cell(3, 0.4,'PO Number', 0, 0, 'L');					$pdf->Cell(0.5, 0.4,':', 0, 0, 'L');		$pdf->Cell(9, 0.4,$po, 0, 0, 'L');								$pdf->Cell(7,0.4,'',0,0,'L');				$pdf->Cell(2.5, 0.4,'PI Number', 0, 0, 'L');	$pdf->Cell(0.5, 0.4,':', 0, 0, 'L');			$pdf->Cell(1, 0.4,'-', 0, 1, 'L');
	...
	if($data->num_rows>0)
	{
		foreach($data->result() as $dataresult)
		{
			...
			if (empty($dataresult->finishing)){
				$pdf->MultiCell(2.5, $tgibaris, '-', 1, 0, 'L', false);											// Desciption finishing empty
			} else {
				$pdf->MultiCell(2.5, $tgibaris, $dataresult->finishing, 1, 0, 'L', false);	// Desciption finishing
			}
