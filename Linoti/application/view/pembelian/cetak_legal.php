<?php
	...
	$pdf->Cell(32.5, 0.7,'PURCHASE ORDER', 1, 1, 'C');
	
	$pdf->SetLineWidth(0.02);
	$pdf->Line(1,3.0,33.5,3.0);
	...
	if($data->num_rows>0)
	{
		foreach($data->result() as $dataresult)
		{
			...
			if ($size==''){
				$pdf->MultiCell(3.5, $barissize, '-', 1, 0, 'L', false);												// Desciption
			} else {
				$pdf->MultiCell(3.5, $barissize, urldecode($size), 1, 0, 'L', false);												// Desciption
			}
