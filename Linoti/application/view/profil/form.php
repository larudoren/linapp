...
$(document).ready(function(){
	...
	$("#simpan").click(function(){
		...
		if(username.length==0){
			$.messager.show({
				title:'Info',
				msg:'Sorry, do not leave Username empty', 
				timeout:2000,
				showType:'show'
			});
			$("#username").focus();
		...
		if(nama_lengkap.length==0){
			$.messager.show({
				title:'Info',
				msg:'Sorry, do not leave Full Name empty', 
				timeout:2000,
				showType:'show'
			});
			$("#nama_lengkap").focus();
...
<tr>    
	<td>Full Name</td>
    <td>:</td>
    <td><input type="text" name="nama_lengkap" id="nama_lengkap"  size="50" maxlength="50" class="easyui-validatebox" data-options="required:true,validType:'length[10,30]'" value="<?php echo $nama_lengkap;?>"/></td>
</tr>
<tr>    
	<td>Password</td>
    <td>:</td>
