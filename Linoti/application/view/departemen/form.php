...
$(document).ready(function(){
	...
	$("#simpan").click(function(){
		...
		if(nama_dept.length==0){
			$.messager.show({
				title:'Info',
				msg:'Sorry, Department Name cannot empty', 
				timeout:2000,
				showType:'show'
			});
			$("#nama_dept").focus();
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/departemen/simpan",
			data	: string,
			cache	: false,
			success	: function(data){
				$.messager.show({
...
<tr>    
    ...
    <td>:</td>
    <td><input type="text" name="nama_dept" id="nama_dept" size="20" class="easyui-validatebox" data-options="required:true" value="<?=$nama_dept?>"/></td>
</tr>
</table>
</fieldset>
