...
class Import_negara extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$d['judul']="Import Negara";			
			
			$d['content'] = $this->load->view('import_negara/form', $d, true);		
	...
	public function import()
	{
		...
		if(!empty($cek)){
			...
			for ($i=4; $i<=$hasildata; $i++)
			{
				...
				$negara = $data->val($i, 2, $sheet=0);
				
				$q = "SELECT COUNT(*) AS tcek FROM negara WHERE negara_code='$kode_negara'";
				$q_cek = $this->app_model->manualQuery($q);
				$h_cek = $q_cek->row();
				if($h_cek->tcek == 0)
				{			 
					$query = "INSERT INTO negara VALUES ('','$kode_negara','$negara', '$user', '$tgl', '$user', '$tgl', '$loc')";
			...
			$d['content'] = $this->load->view('import_negara/view', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
