...
class Po extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$where = " WHERE A.supplier_name<>'' AND A.companyarea='$loc'";
			if(!empty($cari)){
				$where .= " AND A.supplier_name LIKE '%$cari%'";
			} else {
			...
			$text = "SELECT * FROM supplier A  $where ";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$d['tot_hal'] = $tot_hal->num_rows();
			
			$config['base_url'] = site_url() . '/po/index/';
			...
			$d['content'] = $this->load->view('po/view', $d, true);		
	...
	*/
	public function polist(){
		...
		if(!empty($cek)){
			...
			$cari = $this->input->post('txt_cari');
			$d['cari'] = $cari;
			$supplier_code = $this->uri->segment(3);
			$d['supplier_code'] = $supplier_code;
			$text = "SELECT supplier_name FROM supplier WHERE supplier_code='$supplier_code'";
			$data = $this->app_model->manualQuery($text);
			if($data->num_rows() > 0){
				foreach($data->result() as $db){
					$d['dir_name'] = $db->supplier_name;
				}
			} else {
				$d['dir_name'] = '';
			}
			...
			if ($d['dir_name']!=''){			
				$d['content'] = $this->load->view('po/polist', $d, true);		
				$this->load->view('home',$d);
			} else {
				$this->index();
			}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$d['content'] = $this->load->view('po/form', $d, true);		
			$this->load->view('home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function DataDetail()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT A.po, A.kode_barang, B.kode_barang_spc, B.nama_barang, B.nama_barang,  C.unit_name, E.dept_name, A.jmlbeli, D.currency_name, A.hargabeli,A.qty_order
					FROM d_beli A
	...
	*/
	...
	public function showpdf(){
		...
		if(!empty($cek)){
			...
			$d['pdf_file'] = $this->uri->segment(3);
			
			
			$this->load->view('po/showpdf',$d);
		}else{
			header('location:'.base_url());
		}
	}
	...
	public function hapuspdf(){
		...
		if(!empty($cek)){
			...
			$turl = explode(':',urldecode($this->input->post('file')));
			
			unlink("./asset/po/" .$turl[0]."/".$turl[1]);
			//header('location:'.base_url()."index.php/po/polist/".$turl[2]);
			//$this->edit();
		}else{
			header('location:'.base_url());
		}
